...
export const adminsignup = async (req, res, next) => {
  ...
  try {
    ...
    const { number, city, latitude, longitude } = req.body;
    ...
    console.log(branch)
    const fcmToken = branch.fcmToken

    const otp = otpGenerate.generate(6, { upperCaseAlphabets: false, lowerCaseAlphabets: false, specialChars: false })

    const otpnumber = await adminotpmodel.create({ otp: otp, city: city, number: number })
    await otpnumber.save();
    const title = 'KOCART Marchent OTP';
    const body = `Your otp is ${otp}`;
    const url = `http://localhost:5174/`;
    if (!fcmToken) {
      console.log("⚠️ No FCM token for branch");
    } else {
